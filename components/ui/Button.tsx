...
function Button({style, mode = "PRIMARY", children, containerStyle, loading = false, disabled = false, onPress, ...props}: MyButtonProps) {
  ...
  return (
    <View style={[{ backgroundColor: bgColor }, styles.containerStyle, containerStyle]}>
      <Pressable style={[styles.buttonStyle, style, { backgroundColor: disabled ? colors.surfaceDisabled : undefined }]} onPress={myOnPresss} {...props} android_ripple={{color: disabled ? 'transparent' : colors.touchColor, foreground: true}}>
        {
