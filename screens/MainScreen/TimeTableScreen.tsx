...
import {
  calculateNowLineOffset,
  getAllLecturesForDay,
  getColumnWidth,
  hasTimetableUpdated,
  updateLectures,
} from "../../util/timetableUtils";
import { getAllDatesWithLectures } from "../../util/store/database";
import { markDatesForCustomLectures } from "../../util/store/customLectures";
import { Agenda, AgendaList, CalendarProvider, WeekCalendar } from "react-native-calendars";
...
import {
  invalidateLecturesQueries,
  QUERY_LECTURES,
  QUERY_MARKED_DATES,
} from "../../util/http/reactQuery";
...
type TimeTableScreenProps = StaticScreenProps<{
  timelineType: "day" | "week" | "continuous";
}>;
...
function TimeTableScreen({ route }: TimeTableScreenProps) {
  ...
  const { timelineType } = route.params;
  const isWeekView = timelineType === "week";
  ...
  const swipeGesture = Gesture.Pan()
    ...
    .onEnd((event) => {
      ...
      if (translationX > DISTANCE_THRESHOLD && velocityX > VELOCITY_THRESHOLD) {
        ...
        translationX < -DISTANCE_THRESHOLD &&
  ...
  const updateLecturesMutation = useMutation({
    mutationFn: async (forceUpdate: boolean) => {
      ...
      if (!forceUpdate) {
        // if user hasnt swiped down, then this is automaticly checking for updates
  ...
  const { data: lectures } = useQuery<TimetableLecture[]>({
    ...
    queryFn: async () => {
      ...
      if (isWeekView) setWeek(getWeekDates(date)); // stupid
  ...
  useEffect(() => {
    navigation.setOptions({
      title: isWeekView
        ? `${dayjs(week.from).format("D")} - ${dayjs(week.till).format("D MMMM")}`
        : dayjs(date).format("ddd, D MMMM"), // formatWeekDate(week.from, week.till)
    });
  }, [date, week]);
    ...
    timelineType === "continuous" ? (
      ...
      <GestureDetector gesture={swipeGesture}>
        <ScrollView
          ...
          refreshControl={
            <RefreshControl
              refreshing={updateLecturesMutation.isPending}
              onRefresh={onRefresh}
            />
          }
          ref={scrollRef}
        >
          <Timetable
            ...
            renderItem={({ key, ...props }) => (
              <HourSlice
                key={key}
                expand={isWeekView ? 8 : 0}
                {...props}
                onPress={lecturePressed}
                smallMode={isWeekView}
                animationsDisabled={!timetableAnimationsEnabled}
              />
            )}
            ...
            renderHeader={
              isWeekView ? (props) => <TimeTableHeader {...props} /> : undefined
            }
            columnWidth={
              isWeekView
                ? getColumnWidth(windowDimensions, isWeekView)
                : undefined
            }
          />
        </ScrollView>
  ...
  const WeekNum = useMemo(
    () => (
      <AnimatedRollingNumber
        key={theme}
        textStyle={{ fontSize: 16, color: colors.onBackground }}
        value={getSchoolWeekNumber(date)}
      />
    ),
    [date, theme],
  );
  return (
    <Container>
      <LectureDetails
        modalVisible={!!modalLecture}
        lecture={modalLecture!}
        onRequestClose={() => {
          ...
          >
            ...
            <View style={styles.arrowButtonContainerStyle}>
              <IconButton
                ...
                icon={ChevronRight}
                style={{ backgroundColor: "transparent" }}
                onPress={() => setDate(dayjs(date).add(1, "week").toDate())}
              />
            </View>
          </View>
        )}
